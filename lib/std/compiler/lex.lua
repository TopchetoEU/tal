...
local function parse_longlit(ctx, i)
	...
	while true do
		...
		if not find_i or find_i + 1 >= ctx.n then error(find_loc(ctx, ctx.n), "expected ']]'") end
...
local function parse_escape_char(ctx, i, buff)
	...
	elseif ctx.src[i] == chars.u then
		...
		if j >= ctx.n or ctx.src[j] ~= chars.brace_open then error(find_loc(ctx, i), "expected '{'") end
		j = j + 1;

		local j, val = parse_hex(ctx, j);
		if not val then error(find_loc(ctx, i), "expected a hex number") end

		if j >= ctx.n or ctx.src[j] ~= chars.brace_close then error(find_loc(ctx, i), "expected '}'") end
		j = j + 1;
	...
	then
		...
		error(find_loc(ctx, i), "illegal escape sequence in string");
	end
end
--- @param i integer
