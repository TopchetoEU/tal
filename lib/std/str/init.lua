...
function str:unread(ptr, n)
	...
	if not ptr then error "not supported" end
...
function str:write(ptr, n)
	if self._closed then error(errors.closed) end
	if not self._write then error "not supported" end
...
function str:seek(whence, pos)
	if self._closed then error(errors.closed) end
	if not self._seek then error "not supported" end
	return self:_seek(whence or "cur", pos or 0);
end
--- @return std.io.stat
function str:stat()
	if self._closed then error(errors.closed) end
	if not self._stat then error "not supported" end
	return self:_stat();
end
--- @param ... string | integer
function str:chmod(...)
	if self._closed then error(errors.closed) end
	if not self._chmod then error "not supported" end
