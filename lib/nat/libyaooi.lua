...
typedef enum {
	...
	YO_EPROTOTYPE = -91,
	YO_ENOPROTOOPT = -92,
	YO_EPROTONOSUPPORT = -93,
	YO_ESOCKTNOSUPPORT = -94,
	YO_ENOTSUP = -95,
	YO_EPFNOSUPPORT = -96,
	YO_EAFNOSUPPORT = -97,
	YO_EADDRINUSE = -98,
	...
	YO_ENOMEDIUM = -123,
	YO_ECANCELED = -125,
	YO_EAI_BADFLAGS = -1001,
	YO_EAI_NONAME = -1002,
	YO_EAI_AGAIN = -1003,
	YO_EAI_FAIL = -1004,
	YO_EAI_NODATA = -1005,
	YO_EAI_FAMILY = -1006,
	YO_EAI_SOCKTYPE = -1007,
	YO_EAI_SERVICE = -1008,
	YO_EAI_ADDRFAMILY = -1009,
	YO_EAI_MEMORY = -1010,
	YO_EAI_OVERFLOW = -1012,
	YO_EAI_CANCELED = -1101,
	YO_ECHARSET = -2001,
	YO_EUNKNOWN = -3000,
} yo_code_t;
]];
...
local function yo_assert(code)
	if code ~= 0 then error(toerr(code)) end
end
...
function yaooi.socket_bind(addr, port, prot, max_n)
	...
	elseif prot == "udp" then
		...
		sig.error("prot", "invalid protocol");
	end
...
function yaooi.time(clock)
	...
	sig.error("clock", "invalid clock type");
end
...
function yaooi.getpath(type)
	...
	elseif type == "cwd" then
		...
		sig.error("type", "invalid path type");
	end
